...
/*jshint boss:false */
        ...
        */
        _removeChild: function(VNode) {
            ...
            // VNode.domNode && instance.domNode._removeChild(VNode.domNode);
            instance._normalize();
...
if (oldChild.tag==='I-SELECT') {
    console.info('fase 3');
                                ...
                                if (oldChild.isItag && !this._unchangableAttrs && oldChild.itagReady) {
                                    oldChild.itagReady().then(
                                        function() {
if (oldChild.tag==='I-SELECT') {
    console.info('fase 3a');
}
                                            oldChild._setAttrs(newChild.attrs);
                                            oldChild._attrsToModel();
                                            newChild._destroy(true);
                                            newChild.destroyUI(PROTO_SUPPORTED ? null : newChild.__proto__.constructor);
                                        }
                                    );
                                }
                                if (!oldChild.isItag || !this._unchangableAttrs) {
if (oldChild.tag==='I-SELECT') {
    console.info('fase 3b '+oldChild._data.itagRendered);
}
                                    ...
                                    oldChild.isItag || oldChild._setAttrs(newChild.attrs);
